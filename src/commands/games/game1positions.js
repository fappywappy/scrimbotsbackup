...
module.exports = async function (bot, args, msg) {
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    if (i > bot.config.PLACES.length - 2) {
      bot.resources.teams[slot].game1position = 0;
    } else {
      bot.resources.teams[slot].game1position = i + 1;
    }
  }

  for (let i = 0; i < 30; i++) {
    if (givenSlots.some((num) => num === i)) continue;
    bot.resources.teams[i].game1position = 0;
  }

  bot.game1positions = true;
  calculatePoints(bot);
  successMsg(msg, 'Game 1 positions recorded.');
  bot.updateTeams(bot);
}